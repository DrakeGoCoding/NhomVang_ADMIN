    ...
    switch (action.type) {
        ...
        case NEWSLETTER_PAGE_LOADED:
            return {
                ...state,
                inProgress: false,
                subscriberCount: action.payload[0] ? action.payload[0].count : state.subscriberCount,
                data: action.payload[1]
                    ? { ...action.payload[1].newsletter, content: htmlDecode(action.payload[1].newsletter.content) }
                    : state.data
            };
