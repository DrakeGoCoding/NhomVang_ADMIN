            ...
            <Menu>
                {notificationList.map(notification => (
                    <Menu.Item key={notification._id} style={{ width: "inherit" }}>
                        <Link to={notification.link} onClick={() => handleClickNotification(notification._id)}>
                            <div>
                                <Text strong>{notification.user.displayname}</Text> {notification.action}
                            </div>
                ...
                <Menu.Item key="view-all" style={{ textAlign: "center" }} onClick={handleClickViewAll}>
        ...
        <Header className="bg-white sticky top-0 shadow-md z-50 px-7 sm:px-8 md:px-12">
            <Space className="float-right" align="center">
                <Dropdown overlay={menu} placement="bottomRight" trigger={["click"]}>
                    <Badge count={notificationList.length} overflowCount={10} offset={[-10, 10]}>
                        <Button style={{ border: "none" }} shape="circle" size="large" icon={<BellOutlined />} />
                    </Badge>
                </Dropdown>
