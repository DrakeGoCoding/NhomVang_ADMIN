...
export default function NewsletterEditor() {
    ...
    const onUpdateField = (key, value) => store.dispatch({ type: UPDATE_FIELD_NEWSLETTER_EDITOR, key, value });
    ...
    useEffect(() => {
        onLoad(Promise.all([User.countSubscribers(), id ? Newsletter.getById(id) : null]));
    }, [id]);
    ...
    return (
        <div className="newsletter-editor">
            ...
            <Modal title="Newsletter" visible={isModalVisible} onOk={handleSendNewsletter} onCancel={closeModal}>
                <div>
                    Confirm to send this newsletter to all subscribers? (Estimate <Link>{subscriberCount}</Link>)
                </div>
            </Modal>
        </div>
    );
