...
export default function NewsletterPage() {
  const { newsletterList, total, pager, page, inProgress } = useSelector(
    state => state.newsletterList
  );
  ...
  const onReload = () =>
    store.dispatch({ type: FILTER_NEWSLETTERLIST, page, payload: pager(page) });
  ...
  return (
    <Space
      className="newsletter-page max-w-full"
      direction="vertical"
      size="large"
    >
